'use client'

import Image from 'next/image';
import { manufacturers } from '@/constants';
import { useState, Fragment } from 'react';
import { SearchCarFactoryProps } from '@/types'
import { Combobox, Transition } from '@headlessui/react';

function SearchCarFactory({ factory, setFactory}: SearchCarFactoryProps) {
  const [query, setQuery] = useState('');

  const filteredFactories = query === "" 
    ? manufacturers
    : manufacturers.filter((item) => (
        item.toLowerCase().replace(/\s+/g, '')
        .includes(query.toLowerCase().replace(/\s+/g, ''))
    ));

    return (
    <div className='search-manufacture'>
        <Combobox value={factory} onChange={setFactory}>
            <div className='relative w-full'>
                <Combobox.Button className='absolute top-[14px]'>
                    <Image 
                        src='/car-logo.svg'
                        width={20}
                        height={20}
                        className='ml-4'
                        alt='car logo'
                    />
                </Combobox.Button>
                <Combobox.Input 
                    className='search-manufacturer__input'
                    placeholder='Volkswagen'
                    displayValue={(factory: string) => factory}
                    onChange={(e) => setQuery(e.target.value)}
                />

                <Transition 
                    as={Fragment}
                    leave='transition ease-in duration-100'
                    leaveFrom='opacity-100'
                    leaveTo='opacity-0'
                    afterLeave={() => setQuery('')}
                >
                    <Combobox.Options>
                        {filteredFactories.map((item) => (
                            <Combobox.Option
                                key={item}
                                className={({active}) => `
                                relative search-manufacturer__option
                                ${active ? 'bg-green-400 text-white' : 'text-gray-900'}`}
                                value={item}
                            >
                                {item}
                            </Combobox.Option>
                        ))}
                    </Combobox.Options>
                </Transition>

            </div>
        </Combobox>
    </div>
  )
}

export default SearchCarFactory